...
// These must NOT go through tenantIsolation() middleware
// ══════════════════════════════════════════════════════════════════════════

// ── AGENT: POST /api/deployments/agent/heartbeat ──────────────────────────
deployments.post('/agent/heartbeat', async (c) => {
...
// ── AGENT: GET /api/deployments/agent/config ──────────────────────────────
deployments.get('/agent/config', async (c) => {
  const licenceKey = c.req.header('X-Licence-Key');
  if (!licenceKey) return c.json({ error: 'Missing X-Licence-Key' }, 401);
...
// ── AGENT: POST /api/deployments/agent/error ─────────────────────────────
deployments.post('/agent/error', async (c) => {
  const licenceKey = c.req.header('X-Licence-Key');
  if (!licenceKey) return c.json({ error: 'Missing X-Licence-Key' }, 401);
