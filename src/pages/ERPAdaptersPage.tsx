...
export function ERPAdaptersPage() {
 ...
 <div><label className="text-xs t-muted">Connection Name</label><input className="w-full px-3 py-2 rounded-lg border border-[var(--border-card)] text-sm" value={connectForm.name} onChange={e => setConnectForm(p => ({ ...p, name: e.target.value }))} placeholder="Production SAP" /></div>
 ...
 <p className="text-2xl font-bold t-primary mt-1">{connections.length} active</p>
 </Card>
 </div>
