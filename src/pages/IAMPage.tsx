        ...
        <TabPanel>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {roles.map((role) => {
              ...
              const color = role.name.toLowerCase().includes('admin') ? 'text-red-400' : role.name.toLowerCase().includes('exec') ? 'text-[var(--accent)]' : role.name.toLowerCase().includes('manager') ? 'text-[var(--accent)]' : 'text-[var(--accent)]';
              return (
                <Card key={role.id}>
