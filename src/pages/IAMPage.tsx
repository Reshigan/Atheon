              ...
              <Card key={policy.id}>
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <span className="text-[10px] text-neutral-600">Tenant: {policy.tenantId}</span>
                  </div>
                ...
                <div className="mt-3 space-y-2">
                  {(Array.isArray(policy.rules) ? policy.rules : []).map((rule, idx) => {
                    ...
                    return (
                      <div key={idx} className="flex items-center gap-3 p-2 rounded bg-neutral-800/40">
                        {r.effect === 'allow' ? (
                          <Unlock size={14} className="text-emerald-400" />
                        ) : (
                          <Lock size={14} className="text-red-400" />
                        )}
                        <span className="text-xs font-mono text-neutral-300">{String(r.resource || '')}</span>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-xs text-neutral-400">{sso.domainHint}</span>
                    </div>
                  </div>
        ...
        <TabPanel>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {roles.map((role) => {
              ...
              const color = role.name.toLowerCase().includes('admin') ? 'text-red-400' : role.name.toLowerCase().includes('exec') ? 'text-amber-400' : role.name.toLowerCase().includes('manager') ? 'text-blue-400' : 'text-violet-400';
              return (
                <Card key={role.id}>
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    <span className="text-[10px] text-neutral-600">Users</span>
