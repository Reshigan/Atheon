...
const tierConfig: Record<AutonomyTier, { label: string; icon: typeof Eye; color: string }> = {
  'read-only': { label: 'Read-Only', icon: Eye, color: 'text-[var(--accent)]' },
  'assisted': { label: 'Assisted', icon: Wrench, color: 'text-[var(--accent)]' },
...
const statusIcon = (status: string) => {
  ...
  if (status === 'pending') return <Clock size={14} className="text-[var(--accent)]" />;
  if (status === 'approved') return <CheckCircle size={14} className="text-[var(--accent)]" />;
            ...
            {isException && outputData && (
              <div className="p-3 rounded-lg bg-red-500/[0.08] border border-red-500/20">
                ...
                {outputData.suggested_action && (
                  <div className="mt-2 p-2 rounded bg-amber-500/[0.06] border border-[var(--accent)]/20">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div style={{ background: "rgba(18,18,42,0.95)", border: "1px solid rgba(255,255,255,0.1)" }} className="rounded-xl shadow-2xl p-6 w-full max-w-lg space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-white flex items-center gap-2"><Play size={18} className="text-[var(--accent)]" /> Manual Catalyst Execution</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card>
              <h3 className="text-base font-semibold text-white mb-3 flex items-center gap-2">
                <Eye className="w-4 h-4 text-[var(--accent)]" /> Autonomy Tiers
              </h3>
              <div className="space-y-3">
            ...
            <Card>
              <h3 className="text-base font-semibold text-white mb-3 flex items-center gap-2">
                <Shield className="w-4 h-4 text-[var(--accent)]" /> Trust Scores
              </h3>
              <div className="space-y-3">
            ...
            <Card>
              <h3 className="text-base font-semibold text-white mb-3 flex items-center gap-2">
                <Zap className="w-4 h-4 text-[var(--accent)]" /> Governance Metrics
              </h3>
              <div className="space-y-3">
                <div className="p-3 rounded-lg bg-white/[0.04] border border-white/[0.06] backdrop-blur-sm">
                  <span className="text-xs t-secondary">Total Actions</span>
                  <p className="text-lg font-bold text-[var(--accent)]">{governance?.totalActions ?? 0}</p>
                  <p className="text-[10px] text-gray-400">All catalyst executions</p>
                </div>
                <div className="p-3 rounded-lg bg-white/[0.04] border border-white/[0.06] backdrop-blur-sm">
                  <span className="text-xs t-secondary">Pending Approvals</span>
                  <p className="text-lg font-bold text-[var(--accent)]">{governance?.pendingApprovals ?? 0}</p>
