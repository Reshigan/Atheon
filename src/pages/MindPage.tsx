    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div>
        <h2 className="text-lg font-semibold t-primary mb-4 flex items-center gap-2">
          <Layers className="w-4 h-4 text-[var(--accent)]" /> {isAdmin ? 'Inference Tiers' : 'AI Models'}
        </h2>
      ...
      {isAdmin && <div>
        <h2 className="text-lg font-semibold t-primary mb-4 flex items-center gap-2">
          <Cpu className="w-4 h-4 text-[var(--accent)]" /> Training Pipeline
        </h2>
        <Card>
            ...
            {trainingPhases.map((phase) => (
              <div key={phase.name} className="flex items-center gap-4">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                  phase.status === 'completed' ? 'bg-emerald-500/10' : 'bg-[var(--accent)]/10'
                }`}>
      ...
      <div>
        <h2 className="text-lg font-semibold t-primary mb-4 flex items-center gap-2">
          <BarChart3 className="w-4 h-4 text-[var(--accent)]" /> Evaluation Metrics
        </h2>
      ...
      {isAdmin && <Card variant="black">
        <div className="flex items-start gap-3">
          <Database className="w-5 h-5 text-[var(--accent)] mt-0.5 flex-shrink-0" />
          <div>
            <h3 className="text-sm font-semibold t-primary">Architecture Note</h3>
            <p className="text-xs t-muted mt-1">
              Atheon Mind uses a multi-tier inference architecture. The <span className="text-[var(--accent)]">atheon-mind-proxy</span> Worker
